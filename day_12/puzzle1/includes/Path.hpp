...
class Path {
	...
	void	copy_history( void ) {
		for ( std::vector<Path*>::iterator it = _history.begin(); it < _history.end(); it++ ) {
			for ( std::vector<Path*>::iterator it2 = _history.begin(); it2 < _history.end(); it2++ ) {
				if ( (*it2) != (*it) )
					(*it)->add_history( *it2 );
			}
		}
	...
	void	add_path( char* key1, char* key2 ) {
		if ( !strcmp( key1, _key ) ) {
			this->_path.push_back( get_path( key1 ) );
			return ;
		}
		if ( !strcmp( key2, _key ) ) {
			this->_path.push_back( get_path( key2 ) );
			return ;
		}
		for ( std::vector<Path*>::iterator it = _path.begin(); it < _path.end(); it++ ) {
	...
	int	find_path( Path* prev ) {
		...
		for ( std::vector<Path*>::iterator it = _path.begin(); it < _path.end(); it++ ) {
			...
			if ( (*it)->get_size() != START ) {
				if ( strcmp( prev->get_key(), (*it)->get_key() ) ) {
					std::cout << _key << "\t" << (*it)->get_key() << std::endl;
